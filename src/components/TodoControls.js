...
const TodoControls = () => {
    const {tasks, setIsAddingTask, filterTasks, setIsSorting} = useContext(TasksContext)
    ...
    return (
        <div className="todo-controls">
            ...
            <div className="todo-controls__sort">
                <input type="checkbox" className="todo-controls__sort-checkbox" onChange={(e) => setIsSorting(e.target.checked)}/>
                <p className="todo__text">по приоритету</p>
            </div>
            <select className="todo-controls__select" onChange={(e) => filterTasks(e.target.value)}>
                <option value="all">All</option>
                {
                    taskProjects.map(el => <option value={el} key={el}>{el}</option>)
                }
            </select>
        </div>
    );
